...
class test_redis(unittest.TestCase):
    ...
    def test_leaf(self):
        ...
        self.assertEqual(self.session.data_abstraction_layer.paths_set, {"data" + xpath: 'hello'})

        self.assertEqual("hello", self.root.simpleleaf)

        result = self.session.delete(xpath)
        self.assertEqual(result, True)
        self.assertTrue("data"+xpath in self.session.data_abstraction_layer.paths_removed)

        result = self.session.set(xpath, "world")
        self.assertEqual(result, True)
        self.assertEqual(self.session.data_abstraction_layer.paths_set, {"data"+xpath: 'world'})
        self.assertFalse("data"+xpath in self.session.data_abstraction_layer.paths_removed)

        result = self.session.commit()
        self.assertTrue(result)

        self.assertFalse("data"+xpath in self.session.data_abstraction_layer.paths_set)
        self.assertFalse("data"+xpath in self.session.data_abstraction_layer.paths_removed)
